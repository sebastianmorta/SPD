...
def ReadFile(FileName):
    ...
    with open('data\\NEH' + str(FileName) + '.DAT') as file:
        ...
        for i in file:
            if not i.isspace():
                #data.append(Task(i, [int(x) for x in i.split()]))
                data.append([int(x) for x in i.split()])

            #
            #     p, w, d = i.split()
            #     data.append(Task(int(p), int(w), int(d)))
    return data,n,m
...
def Neh(data, n, m):
    ...
    for i in range(1,n):
        ...
        for j in range(i+1):
            ...
            temp_cmax = Cmax(data, n, m)
            if cm > temp_cmax[len(temp_sequence)-1][m]:
                best_sequence = temp_sequence
                cm = temp_cmax[len(temp_sequence)][m-1]
